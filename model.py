    ...
    def run_epoch(self, train_iterator, val_iterator, epoch):
       ...
       # val_losses = []
        ...
        for i, batch in enumerate(train_iterator):
            ...
            print("\tAverage train Accuracy: {:.4f}".format(train_accuracy))
            #self.train()
            if i % 100 == 0:
                print("Iter: {}".format(i+1))
                avg_train_loss =1+(np.mean(losses))
                train_losses.append(avg_train_loss)
                print("\tAverage training loss: {:.5f}".format(avg_train_loss))
               # val_losses.append(loss)
               # print("\tAverage val loss: {:.5f}".format(val_losses))
                losses = []
        ...
        for i, batch in enumerate(val_iterator):
            ...
            if i % 100 == 0:
                #print("Iter: {}".format(i+1))
                avg_valid_loss =1+(np.mean(valid_losses))
                valid_losses.append(avg_valid_loss)
                print("\tAverage validation loss: {:.5f}".format(avg_valid_loss))
               # val_losses.append(loss)
               # print("\tAverage val loss: {:.5f}".format(val_losses))
               #valid_losses = []
                
        return train_losses,val_accuracies, train_accuracy, valid_losses#,val_losses
